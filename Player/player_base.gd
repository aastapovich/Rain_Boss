...
const MovementControllerScript = preload("res://Player/movement_controller.gd")
...
var wheelbarrow_level: int = 0  ## Уровень апгрейда тачки (вместимость — из InventoryConfig)
...
# ══════════════════════════════════════════════════════════════════════════════
var wheelbarrow_capacity: int = 100  ## Максимальный объём (л.) — синхронизируется с Globals.wheelbarrow_capacity_max
...
## Контракт для Carriable/Wheel: читается через get("_last_facing").
...
func _setup_unload_controller() -> void:
	if not unload_ctrl:
		return
	var wheel: Node = get_tree().root.find_child("Wheel", true, false) if get_tree() else null
	unload_ctrl.setup(player_anim, wheel)
...
func _process_move(delta: float) -> void:
	...
	var wb := get_tree().root.find_child("Wheelbarrow", true, false) if get_tree() else null
...
func _set_facing(left: bool) -> void:
	if _facing_left == left:
		return
	_facing_left = left
	_last_facing = -1 if left else 1
	player_anim.flip_h     = left
	player_anim.position.x = _sprite_base_x + (hand_align_left if left else hand_align_right)
	# Уведомляем удерживаемый предмет (Carriable/Wheel) о смене направления.
	var _item_inv := get_node_or_null("PlayerInventory")
	if _item_inv:
		var held = _item_inv.get("held_item")
		if held and held.has_method("set_facing"):
			held.set_facing(_last_facing)

## Проксирует вызов из Area2D предметов в PlayerInventory.
func set_nearby_item(item, source = null) -> void:
	var _item_inv := get_node_or_null("PlayerInventory")
	if _item_inv:
		_item_inv.set_nearby_item(item, source)
...
## Обновляет скорость по заполнению тачки — подписка на Events.wheelbarrow_changed.
func _on_wb_changed(_amount: int) -> void:
	var wb := get_tree().root.find_child("Wheelbarrow", true, false) if get_tree() else null
	var speed_coef: float = wb.speed_coef if (wb and "speed_coef" in wb) else 1.0
	speed = speed_norm * speed_coef
...
func _on_stop_player_body_entered(body: Node2D) -> void:
	...
	if state != State.MOVE:
		return
	# Delegate unload behaviour to InventorySystem (adapter).
	var inv := _get_inv()
	...
	if inv:
		...
		_change_state(State.MOVE)
		return
	# Fallback: InventorySystem недоступен — используем unload_ctrl напрямую.
	if Globals.point_wheelbarrow > 0:
		...
		XpManager.on_water_unloaded(water_before)
		Events.exit_point_changed.emit(exit_point, max_exit_point)
		_change_state(State.MOVE)
		if exit_point >= max_exit_point:
			var game := get_tree().root.get_node_or_null("Game")
