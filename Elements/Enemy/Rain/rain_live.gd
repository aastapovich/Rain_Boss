## Живительная капля: лечит игрока (+20 HP), вода в тачку (+10), на земле — без лужи.
...
## Дефолтная таблица попаданий для живительной капли.
...
func _add_default_hit_configs() -> void:
	...
	ground.call_args  = ["live", "_pos_"]
	ground.do_splash  = true
	ground.do_puddle  = false
	hit_configs.append(ground)

	# Тачка → +10 воды (do_splash=false: wheelbarrow.gd.rain_wheelbarrow_add сам вызывает _play_splash)
	var wheelbarrow := _HitCfg.new()
	wheelbarrow.key_method = "rain_wheelbarrow_add"
	wheelbarrow.call_args  = [10, "live"]
