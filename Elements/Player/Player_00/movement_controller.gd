...
func process_move(delta: float) -> void:
	...
	if player and player.get_tree():
		inv = player.get_tree().get_root().get_node_or_null("Game/Progression/InventorySystem")
	...
	if inv and inv.has_method("set_wheelbarrow_moving"):
		inv.set_wheelbarrow_moving(player, moving)
	# else: no-op — Player should not directly manipulate wheelbarrow internals

	player.previous_direction = player.direction
	player.direction          = Input.get_axis("Left", "Right")
	player.velocity.x         = player.direction * max(0.0, player.speed - player.wb_speed_penalty)
